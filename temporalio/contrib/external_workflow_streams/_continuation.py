...
class Continuation:
    ...
    ``stream_names`` is carried alongside so a renumbered subscription is
    detectable. Restoring a cursor onto a differently-numbered wait would resume
    one stream at another's offset, which the backend would accept and no later
    check would catch.
    """
...
def encode_continuation(continuation: Continuation) -> bytes:
    ...
    # Sorted, so the same state always encodes to the same bytes: this rides on a
    # command whose payload replay compares against History.
...
def decode_continuation(raw: bytes) -> Continuation:
    ...
    return Continuation(cursors=cursors, stream_names=stream_names)
